...
### Яндекс: «Переобход страниц» — некорректный URL

В поле **не вводите** `https://z-tech.pro/...` — только относительные пути (по одному на строку):
...
Если ошибка остаётся: **Настройки → Основные** — сайт должен быть **`https://z-tech.pro`** (без `www`, с `https`). Если добавлен как `http://` или `https://www.z-tech.pro`, переобход с `https://z-tech.pro` не примет.

Sitemap в Вебмастере: `https://z-tech.pro/sitemap.xml`
